            ...
            <table class="text-left w-full border-collapse">
                ...
                foreach ($listeUser as $user) {
                    ...
                    <tr class="hover:bg-grey-lighter">
                        <td value="<?php $user['Id'] ?>" name="idUser" class="p-4 w-1/4"> <?php echo((isset($user['Last_name']) && $user['Last_name'] <> "" && $user['Last_name'] <> NULL) ? $user['Last_name'] : "") ?> <?php echo((isset($user['First_name']) && $user['First_name'] <> "" && $user['First_name'] <> NULL) ? $user['First_name'] : "-") ?></td>
                        <td class="p-4 w-1/4">
                            <select>
                                <?php
                                foreach ($listeClasses as $classes) {
                                    ?>
                                    <option value="<?php $classes['Id'] ?>" name="idClasses"> <?php echo $classes['Matter'] ?></option>
                                    <?php
                                }
                                ?>
                            </select>
                        ...
                        <td class="py-4 px-6 border-b border-grey-light"> <input name="notes"  class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="note" type="text" placeholder="Saisir note"> </td>
                    </tr>
                    <?php
                }
                ?>
